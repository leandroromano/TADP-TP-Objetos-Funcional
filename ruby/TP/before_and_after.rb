...
class Module
  def before_and_after_each_call(before, after)
    ...
    define_singleton_method :method_added do |method| # modifico el metodo de clase "method_added"
      sincronizar_con_superclase
      redefinir_metodo method # redefino el metodo con las caracteristicas pedidas
    end
  end

  def sincronizar_con_superclase
    if @overriden_methods.nil? # solo va a pasar cuando se trate de una subclase de la clase que definio el before_and_after, y esta no defina ningun before_and_after
      ...
      @pres_and_posts_chequeando = superclass.instance_variable_get :@pres_and_posts_chequeando
    end
  end
